...
3.根据用户名或者id生成jwt
4.将token响应给其前端
5.登录后请求需要在请求头中携带token
6.获取请求头中token进行解析得到userid
7.根据用户信息查询相关权限放行
8.返回目标资源响应给前端
```
...
```shell
#Spring Security过滤器链路
DefaultSecurityFilterChain

#interface:AbstractAuthenticationProcessingFilter
#impl：UsernamePasswordAuthenticationFilter
负责处理登录请求

#ExceptionTranslationFilter
处理过滤器链路中AuthenticationException和AccessDeniedException异常情况
...
- 自定义登录接口 - 调用ProviderManager的方法进行认证，认证通过生成jwt存储于redis
- 自定义UserDetailsService-在数据库中查询用户信息
...
- 定义jwt认证过滤器
  ...
  - 存入SecurityContextHolder(每一次存储SecurityContext都是全新的对象)

有一些应用是不使用Session机制的，比如无状态的Restful API,这类接口需要每次认证。但是对于这种请求的处理过程，SecurityContextPersistenceFilter还是会被使用到，它被用来在每个请求处理结束时清除SecurityContextHolder中的安全上下文
